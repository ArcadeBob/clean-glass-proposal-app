...
import { useTextareaSanitization } from '../../hooks/useInputSanitization';
...
export const SanitizedTextarea: React.FC<SanitizedTextareaProps> = ({
  ...
  const { value, isValid, errors, warnings, handleChange, handleInputChange } =
    useTextareaSanitization(externalValue || '', sanitizationOptions);
  ...
  // Determine textarea styling based on validation state
  const getTextareaClassName = () => {
    let baseClass = `w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors duration-200 resize-vertical ${className}`;
      ...
      {showValidation && (
        <div className="space-y-1">
          ...
          {sanitizationOptions.maxLength && (
            <p
              className={`text-xs ${value.length > sanitizationOptions.maxLength * 0.9 ? 'text-red-500' : 'text-gray-500'}`}
            >
              {value.length} / {sanitizationOptions.maxLength} characters
            </p>
          )}
        </div>
      )}
